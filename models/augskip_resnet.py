...
class AugSkipResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, pre_ac=False, bn=False, drop_conv=0, drop_fc=0):
        ...
        for m in self.modules():
            if isinstance(m, AugSkipBottleneck_no_bn) or isinstance(m, AugSkipBottleneck):
                ...
                nn.init.constant_(m.conv3.bias, 0)
                if m.downsample is not None:
                    partial_identity_conv1x1(m.downsample[0].weight)
                    nn.init.constant_(m.downsample[0].bias, 0)

...
def conv3x3(in_planes: int, out_planes: int, stride: int = 1, groups: int = 1, dilation: int = 1, base_conv: nn.Conv2d = Conv2d) -> nn.Conv2d:
    """3x3 convolution with padding"""
    return base_conv(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=dilation, groups=groups, bias=True, dilation=dilation)


def conv1x1(in_planes: int, out_planes: int, stride: int = 1, base_conv: nn.Conv2d = Conv2d) -> nn.Conv2d:
    """1x1 convolution"""
    return base_conv(in_planes, out_planes, kernel_size=1, stride=stride, bias=True)
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = activation_fn[self.activation](x=x)
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = activation_fn[self.activation](x)
...
class NFResNet(nn.Module):
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
                    stride: int = 1, dilate: bool = False, alpha: float = 0.2, beta: float = 1.0, activation: str = 'relu', base_conv: nn.Conv2d = Conv2d) -> nn.Sequential:
        downsample = None
        ...
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, alpha=alpha, beta=beta, activation=activation, base_conv=base_conv, block_idx=self.block_idx, max_block=self.max_depth))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                alpha=alpha, beta=beta, activation=activation,
                                base_conv=base_conv, block_idx=self.block_idx, max_block=self.max_depth,))
...
def nf_resnet18(alpha: float = 0.2, beta: float = 1.0, activation: str = 'relu', base_conv: nn.Conv2d = Conv2d, **kwargs: Any) -> NFResNet:
...
def nf_resnet34(alpha: float = 0.2, beta: float = 1.0, activation: str = 'relu', base_conv: nn.Conv2d = Conv2d, **kwargs: Any) -> NFResNet:
...
def nf_resnet50(alpha: float = 0.2, beta: float = 1.0, activation: str = 'relu', base_conv: nn.Conv2d = Conv2d, **kwargs: Any) -> NFResNet:
...
def nf_resnet101(alpha: float = 0.2, beta: float = 1.0, activation: str = 'relu', base_conv: nn.Conv2d = Conv2d, **kwargs: Any) -> NFResNet:
...
def nf_resnet152(alpha: float = 0.2, beta: float = 1.0, activation: str = 'relu', base_conv: nn.Conv2d = Conv2d, **kwargs: Any) -> NFResNet:
...
def nf_resnext50_32x4d(alpha: float = 0.2, beta: float = 1.0, activation: str = 'relu', base_conv: nn.Conv2d = Conv2d, **kwargs: Any) -> NFResNet:
...
def nf_resnext101_32x8d(alpha: float = 0.2, beta: float = 1.0, activation: str = 'relu', base_conv: nn.Conv2d = Conv2d, **kwargs: Any) -> NFResNet:
...
def nf_wide_resnet50_2(alpha: float = 0.2, beta: float = 1.0, activation: str = 'relu', base_conv: nn.Conv2d = Conv2d, **kwargs: Any) -> NFResNet:
...
def nf_wide_resnet101_2(alpha: float = 0.2, beta: float = 1.0, activation: str = 'relu', base_conv: nn.Conv2d = Conv2d, **kwargs: Any) -> NFResNet:

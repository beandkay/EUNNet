...
class NFNet(nn.Module):
    def __init__(self, num_classes:int, variant:str='F0', stochdepth_rate:float=None, 
        ...
        self.final_conv = Conv2d(in_channels=out_channels, out_channels=final_conv_channels, kernel_size=1)
...
class Stem(nn.Module):
    def __init__(self, activation:str='gelu'):
        ...
        self.activation = activations_dict[activation]
        self.conv0 = Conv2d(in_channels=3, out_channels=16, kernel_size=3, stride=2)
        self.conv1 = Conv2d(in_channels=16, out_channels=32, kernel_size=3, stride=1)
        self.conv2 = Conv2d(in_channels=32, out_channels=64, kernel_size=3, stride=1)
        self.conv3 = Conv2d(in_channels=64, out_channels=128, kernel_size=3, stride=2)
...
class NFBlock(nn.Module):
    def __init__(self, in_channels:int, out_channels:int, expansion:float=0.5, 
        ...
        self.conv0 = Conv2d(in_channels=self.in_channels, out_channels=self.width, kernel_size=1)
        self.conv1 = Conv2d(in_channels=self.width, out_channels=self.width, kernel_size=3, stride=stride, padding=1, groups=self.groups)
        self.conv1b = Conv2d(in_channels=self.width, out_channels=self.width, kernel_size=3, stride=1, padding=1, groups=self.groups)
        self.conv2 = Conv2d(in_channels=self.width, out_channels=self.out_channels, kernel_size=1)
        ...
        if self.use_projection:
            ...
            self.conv_shortcut = Conv2d(self.in_channels, self.out_channels, kernel_size=1)
...
def nfnet_f0(num_classes:int, variant:str='F0', stochdepth_rate:float=0.25, 
        alpha:float=0.2, se_ratio:float=0.5, activation:str='gelu', drop_conv=None, drop_fc=None):
    return NFNet(num_classes, variant, stochdepth_rate, alpha, se_ratio, activation)

def nfnet_f1(num_classes:int, variant:str='F1', stochdepth_rate:float=0.25, 
        alpha:float=0.2, se_ratio:float=0.5, activation:str='gelu', drop_conv=None, drop_fc=None):
    return NFNet(num_classes, variant, stochdepth_rate, alpha, se_ratio, activation)

def nfnet_f2(num_classes:int, variant:str='F2', stochdepth_rate:float=0.25, 
        alpha:float=0.2, se_ratio:float=0.5, activation:str='gelu', drop_conv=None, drop_fc=None):
    return NFNet(num_classes, variant, stochdepth_rate, alpha, se_ratio, activation)

def nfnet_f3(num_classes:int, variant:str='F3', stochdepth_rate:float=0.25, 
        alpha:float=0.2, se_ratio:float=0.5, activation:str='gelu', drop_conv=None, drop_fc=None):
    return NFNet(num_classes, variant, stochdepth_rate, alpha, se_ratio, activation)

def nfnet_f4(num_classes:int, variant:str='F4', stochdepth_rate:float=0.25, 
        alpha:float=0.2, se_ratio:float=0.5, activation:str='gelu', drop_conv=None, drop_fc=None):
    return NFNet(num_classes, variant, stochdepth_rate, alpha, se_ratio, activation)

def nfnet_f5(num_classes:int, variant:str='F5', stochdepth_rate:float=0.25, 
        alpha:float=0.2, se_ratio:float=0.5, activation:str='gelu', drop_conv=None, drop_fc=None):
    return NFNet(num_classes, variant, stochdepth_rate, alpha, se_ratio, activation)

def nfnet_f6(num_classes:int, variant:str='F6', stochdepth_rate:float=0.25, 
        alpha:float=0.2, se_ratio:float=0.5, activation:str='gelu', drop_conv=None, drop_fc=None):
    return NFNet(num_classes, variant, stochdepth_rate, alpha, se_ratio, activation)